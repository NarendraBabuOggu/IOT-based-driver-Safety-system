...
def savePhoto(fram):
    ...
    picname = picname+'.jpg'
    cv2.imwrite(picname, fram)
    upload(picname)
    return                
                
def upload(picname):
    print ("Saving Photo")
    url = ''#url of the cloud being used
...
while True:
    ...
    for rect in rects:
        ...
        cv2.putText(frame, "Eye closing Time:"+str(blink), (10, 80),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
        
        if ear < EYE_AR_THRESH:
        #if 0.18 < ear > EYE_AR_THRESH:
            
            #cv2.putText(frame, "Driver is Not Sleeping", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
            COUNTER = COUNTER+1
            if 10 < COUNTER < 30:
                cv2.putText(frame, "Driver is going to Sleep!!!WARNING!!!", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 120), 2)
            elif COUNTER >30 :
                cv2.putText(frame, "ALARM!!! ALARM!!! DRIVER IS SLEEPING", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                playAudio()
                savePhoto(frame)
            ...
            COUNTER = 0
            blink=0
            cv2.putText(frame, "Driver is Not Sleeping", (10, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2)
